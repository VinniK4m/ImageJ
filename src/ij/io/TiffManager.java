...
public class TiffManager {
	...
	public TiffManager(String directory, String name) {
		if (directory==null)
			directory = "";
	...
	public TiffManager(InputStream in, String name) {
		directory = "";
		this.name = name;
		url = "";
	...
	public TiffManager(FileInfo fi) {
	}
	...
	public void TiffEncoder (FileInfo fi) {
		this.fi = fi;
		fi.intelByteOrder = littleEndian;
		bitsPerSample = 8;
		samplesPerPixel = 1;
	...
	void writeEntry(OutputStream out, int tag, int fieldType, int count, int value) throws IOException {
		...
		writeInt(out, count);
		if (count==1 && fieldType==TiffManager.SHORT) {
	...
	void writeIFD(OutputStream out, int imageOffset, int nextIFD) throws IOException {
		...
		writeEntry(out, TiffManager.NEW_SUBFILE_TYPE, 4, 1, 0);
		writeEntry(out, TiffManager.IMAGE_WIDTH, 4, 1, fi.width);
		writeEntry(out, TiffManager.IMAGE_LENGTH, 4, 1, fi.height);
		if (fi.fileType==FileInfo.RGB||fi.fileType==FileInfo.RGB48) {
			writeEntry(out, TiffManager.BITS_PER_SAMPLE,  3, 3, tagDataOffset);
			tagDataOffset += BPS_DATA_SIZE;
		} else
			writeEntry(out, TiffManager.BITS_PER_SAMPLE,  3, 1, bitsPerSample);
		writeEntry(out, TiffManager.COMPRESSION,  3, 1, 1);	//No Compression
		writeEntry(out, TiffManager.PHOTO_INTERP, 3, 1, photoInterp);
		if (description!=null) {
			writeEntry(out, TiffManager.IMAGE_DESCRIPTION, 2, description.length, tagDataOffset);
			tagDataOffset += description.length;
		}
		writeEntry(out, TiffManager.STRIP_OFFSETS,    4, 1, imageOffset);
		writeEntry(out, TiffManager.SAMPLES_PER_PIXEL,3, 1, samplesPerPixel);
		writeEntry(out, TiffManager.ROWS_PER_STRIP,   3, 1, fi.height);
		writeEntry(out, TiffManager.STRIP_BYTE_COUNT, 4, 1, imageSize);
		if (fi.unit!=null && fi.pixelWidth!=0 && fi.pixelHeight!=0) {
			writeEntry(out, TiffManager.X_RESOLUTION, 5, 1, tagDataOffset);
			writeEntry(out, TiffManager.Y_RESOLUTION, 5, 1, tagDataOffset+8);
			...
			if (fi.unit.equals("inch"))
				...
				unit = 3;
			writeEntry(out, TiffManager.RESOLUTION_UNIT, 3, 1, unit);
		}
		if (fi.fileType==fi.GRAY32_FLOAT) {
			int format = TiffManager.FLOATING_POINT;
			writeEntry(out, TiffManager.SAMPLE_FORMAT, 3, 1, format);
		}
		if (colorMapSize>0) {
			writeEntry(out, TiffManager.COLOR_MAP, 3, MAP_SIZE, tagDataOffset);
			tagDataOffset += MAP_SIZE*2;
		}
		if (metaDataSize>0) {
			writeEntry(out, TiffManager.META_DATA_BYTE_COUNTS, 4, nMetaDataEntries, tagDataOffset);
			writeEntry(out, TiffManager.META_DATA, 1, metaDataSize, tagDataOffset+4*nMetaDataEntries);
	...
	void writeMetaData(OutputStream out) throws IOException {
		...
		writeInt(out, TiffManager.MAGIC_NUMBER); // "IJIJ"
		if (fi.info!=null) {
			writeInt(out, TiffManager.INFO); // type="info"
			writeInt(out, 1); // count
		}
		if (nSliceLabels>0) {
			writeInt(out, TiffManager.LABELS); // type="labl"
			writeInt(out, nSliceLabels); // count
		}
		if (fi.displayRanges!=null) {
			writeInt(out, TiffManager.RANGES); // type="rang"
			writeInt(out, 1); // count
		}
		if (fi.channelLuts!=null) {
			writeInt(out, TiffManager.LUTS); // type="luts"
			writeInt(out, fi.channelLuts.length); // count
		}
		if (fi.plot!=null) {
			writeInt(out, TiffManager.PLOT); // type="plot"
			writeInt(out, 1); // count
		}
		if (fi.roi!=null) {
			writeInt(out, TiffManager.ROI); // type="roi "
			writeInt(out, 1); // count
		}
		if (fi.overlay!=null) {
			writeInt(out, TiffManager.OVERLAY); // type="over"
			writeInt(out, fi.overlay.length); // count
		}
		if (fi.properties!=null) {
			writeInt(out, TiffManager.PROPERTIES); // type="prop"
